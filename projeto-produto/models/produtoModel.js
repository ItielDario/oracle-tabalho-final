    ...
    async listarProdutos() {
        ...
        if (rows.length > 0) {
            for (let i = 0; i < rows.length; i++) {
                ...
                if (prdImagem != null && prdImagem !== "") {
                    // Verifica se é um caminho local, caso contrário, assume que é um endereço remoto válido
                    if (!prdImagem.startsWith('http') && 
                        fs.existsSync(global.RAIZ_PROJETO + "/public" + global.PRODUTO_IMG_CAMINHO + prdImagem) == false) {
                        prdImagem = "sem-imagem.png";
                    }
                } else {
                    prdImagem = "sem-imagem.png";
